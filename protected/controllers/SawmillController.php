...
class SawmillController extends Controller
{
	...
	public function actionAdminIndex($partial = false){
        ...
        $filter = new Sawmill('filter');

		if (isset($_GET['Sawmill'])){
            $filter->attributes = $_GET['Sawmill'];
        }
		...
		$params = array(
			...
			"filter" => $filter,
			"count" => $count,
			"labels" => Sawmill::attributeLabels(),
		);
	...
	public function actionAdminCreate()
	{
		...
		if(isset($_POST["Sawmill"])) {
			if( $model->updateObj($_POST["Sawmill"]) ){
	...
	{
		$model = Sawmill::model()->findByPk($id);
