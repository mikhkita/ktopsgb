...
<table class="b-table" border="1">
	<tr>
		...
		<th><? echo $labels["group_id"]; ?></th>
		<th style="width: 100px;">Действия</th>
	</tr>
	<tr class="b-filter">
		<td></td>
		<td><?php echo CHtml::activeTextField($filter, 'name', array('tabindex' => 1, "placeholder" => "Поиск по наименованию")); ?></td>
		<td><?php echo CHtml::activeDropDownList($filter, 'group_id', CHtml::listData(PlankGroup::model()->findAll(), 'id', 'name'), array("class" => "select2", "empty" => "Все группы", "tabindex" => 1)); ?></td>
	...
	<? foreach ($data as $i => $item): ?>
		<tr>
			...
			<td class="align-left"><? echo $item->group->name; ?></td>
			<td>
