<? if( $this->checkAccess("updateBranch", $container->station->branch_id) ): ?><a href="<?php echo $this->createUrl("/".$this->adminMenu["cur"]->code."/admincargocreate",array("container_id" => $_GET["container_id"]))?>" class="ajax-form ajax-create b-butt b-top-butt">Добавить</a><? endif; ?>
...
<table class="b-table" border="1">
	<tr>
		...
		<th><?=$labels["cubage"]?></th>
		<th><?=$labels["form_id"]?></th>
		<th><?=$labels["count"]?></th>
		<th><?=$labels["price"]?></th>
		<th><?=$labels["sum"]?></th>
		<th><?=$labels["currency_id"]?></th>
	...
	<? if( count($data) ): ?>
		<? foreach ($data as $i => $item): ?>
			<tr>
				...
				<td><?=$item->count?></td>
				<td><?=$item->price?></td>
				<td class="tr"><?=number_format( $item->price*$item->cubage, 2, '.', '&nbsp;' )?></td>
				<td><?=$item->currency?></td>
				<td>
					<? if( $this->checkAccess("updateBranch", $container->station->branch_id) ): ?>
