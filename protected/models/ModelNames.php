...
class ModelNames extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array("sort", "parent", "numerical", "integerOnly" => true),
			array("code, name, vin_name, rod_name", "length", "max" => 128),
			array("rule", "length", "max" => 32),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array("id, code, name, vin_name, rod_name, rule, sort, parent", "safe", "on" => "search"),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			"parent" => array(self::BELONGS_TO, "ModelNames", "parent"),
			"submenu" => array(self::HAS_MANY, "ModelNames", "parent", "order" => "submenu.sort ASC"),
		);
	}
